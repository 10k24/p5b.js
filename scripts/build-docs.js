...
for (const version of ["v1", "v2"]) {
    fs.readdirSync(path.join(stubsDir, version)).forEach(file => {
        if (file.startsWith("readme-") && file.endsWith(".js")) {
            const name = file.replace(/^readme-/, "").replace(/\.js$/, "");
            stubs[version][name] = escapeHtml(fs.readFileSync(path.join(stubsDir, version, file), "utf8").trim());
        }
    });
}

// Disable doT's internal logger; emit each template's "Compiling" line next to its
// output path so the log reads top-down in the order things are compiled.
dot.log = false;
...
writeOutput("README.md", cleanedReadme);

// Compile the examples' package.json manifests from the single .dot template.
// Root package.json is the source of truth for the package name and version; the
// per-version config below is the only other irreducible data (dir + explicit p5 range,
...
const writeManifest = (dest, data) => writeOutput(dest, `${examplesPkg(data)}\n`);
...
for (const { dir, p5 } of exampleVersions) {
    ...
    writeManifest(`examples/${dir}/package.json`, {

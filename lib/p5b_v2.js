...
class P5b extends P5bBase {
    ...
    toFrame() {
        ...
        // Scale path: WebGL canvas can't be drawImage'd directly; read pixels via
        // loadPixels and copy into a temp 2D canvas. 2D canvas is used as-is.
        const isP3D = Boolean(this._myP5._renderer?.isP3D);
        let srcDrawable;
        if (isP3D) {
            this._myP5.loadPixels();
            // Cache the read-back canvas (recreated on size change) per frame.
    ...
    _initSketch() {
        ...
        global.redraw = (...args) => {
            // p5 2.x redraw() is async; keep _redrawing set until it settles so the
            // stopped-state guard in _wrappedDraw doesn't block the redraw frame.
    ...
    _bindGlobals() {
        ...
        // legacy onSuccess/onError callbacks still fire.
        ...
        global.loadStrings = async (filePath, callback, errorCallback) => {
            try {
                // v2 returns a Promise of lines (URLs + local files via the DOM shim).
        ...
        global.loadBytes = async (file, callback, errorCallback) => {
            try {
                // v2 returns a Promise<Uint8Array>.
        ...
        // loadShader/loadModel/loadBlob delegate to native p5 via file:// URLs.

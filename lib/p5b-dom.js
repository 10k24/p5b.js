...
class P5bDOM {
    ...
    _init() {
        ...
        // p5 v2 WebGL shader strands temporarily inject hook functions (e.g.
        // getPixelInputs) via `window[name] = fn`. In a browser these land on the
        // real global; here we forward them to Node.js's global so they're accessible
        // as bare names inside modify(). This proxy is v2-only: p5 v1 has no 3D/shaders
        // by design, and its _globalInit would see a window.setup/window.draw that
        // falls back to global and spawn a duplicate global-mode instance. Use the
        // plain win object for v1 so those names stay undefined.
        if (this.p5Version === 2) {

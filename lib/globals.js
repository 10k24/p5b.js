...
// Resolve the installed major version of a p5 package (default "p5").
// Returns 1 or 2. Only reads the package.json; never loads the p5 module, so
// this stays a leaf module (no circular dependency with p5b-base/p5b.js).
const findP5Version = (pkg = process.env.P5B_P5_PKG || "p5") => {
    ...
    if (major !== 1 && major !== 2) {
        throw new Error(`Unsupported p5.js version "${version}" (expected 1.x or 2.x).`);
    }
    return major;
};
...
module.exports = { noop, isAsyncFunction, mathFunctions, p5Constants, noopFunctions, findP5Version };
